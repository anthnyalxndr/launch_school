...
/*
Problem Description: Write a program that can calculate the Hamming distance
...
PEDAC:
Understand:
  - compute hamming distances when input is valid
Examples / Test Cases:
  - should we expect invalid base nucleotides? No

Data Structures: sequences are strings.
*/

"use strict";
class DNA {
  constructor(DNAStrand) {
    this.sequence = DNAStrand;
  }

  hammingDistance(DNAStrand) {
    let currentStrand = this.sequence;
    let comparisonStrand = DNAStrand;
    let minLength = Math.min(currentStrand.length, comparisonStrand.length);
    let hammingDistance = 0;
    for (let idx = 0; idx < minLength; idx += 1) {
      let currentStrandBase = currentStrand[idx];
      let newStrandBase = comparisonStrand[idx];
      if (currentStrandBase !== newStrandBase) hammingDistance += 1;
    }
