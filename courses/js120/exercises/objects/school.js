...
function createStudents() {
  // Examples of created student objects with grades; methods on the objects
...
function createSchool() {
  return {
    ...
    courseReport(courseName) {
      const allGrades = [];
      const numOfStudentsTakingCourse = this.students.filter(({courses}) => {
        return courses.find(({name}) => name === courseName);
      }).length;
      if (numOfStudentsTakingCourse === 0) return;
      console.log(`=${courseName} Grades=`);
      this.students.forEach(({name, courses}) => {
        const {grade} = courses.find(({name}) => name === courseName);
        if (grade) {
          console.log(`${name}: ${grade}`);
...
function schoolTestCases() {
  ...
  const [foo, bar, qux] = createStudents();
